...
class ComputedPurchaseOrder(models.Model):
    ...
    compute_pvi_quantity = fields.Boolean("PVI quantity taken in account", default=True)

    compute_pvi_d_quantity = fields.Boolean("Draft PVI taken in account", default=True)
    ...
    def write_active_product_stock(self, cpo, cpol_list):
        ...
        for cpol in cpol_list:
            ...
            cpol[2].update(
                {
                    "average_consumption": product_id.custom_average_consumption(
                        parametres, pvi
                    )[1]
                    or 0.0
                }
            )
        super(ComputedPurchaseOrder, self).write_active_product_stock(cpo, cpol_list)
