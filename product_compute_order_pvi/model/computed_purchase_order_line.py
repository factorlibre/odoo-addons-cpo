...
class ComputedPurchaseOrderLine(models.Model):
    ...
    pvi_draft_qty = fields.Float("PVI Draft Outgoing Quantity", help="Draft sales")
    pvi_qty = fields.Float("PVI Outgoing Quantity", help="Draft sales")
        ...
        for cpol in self:
            if cpol.product_id.id:
                ...
                if cpol.computed_purchase_order_id.compute_pvi_d_quantity:
                    line_dict["pvi_draft_qty"] = product_id.custom_average_consumption(
                        parametres, pvi
                    )[0]
                if cpol.computed_purchase_order_id.compute_pvi_quantity:
                    parametres = ["pvi_confirmed"]
                    line_dict["pvi_qty"] = product_id.custom_average_consumption(
                        parametres, pvi
                    )[0]
